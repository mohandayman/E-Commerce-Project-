...
{
    public class ProductRepository<T> : IproductRepository<T> where T : class, Iproduct
    {
        public Context SystemContext { get; }

        public ProductRepository( Context systemContext)
        {
        ...
        {
            return await SystemContext.Set<T>().FirstOrDefaultAsync(I => I.ProductCode == code);
        }

        public async Task<bool> Add(T Element)
        {
            Element.ProductCode = Guid.NewGuid().ToString();
        ...
        {
            T Entity = await GetByCode(UpdatedValue.ProductCode);
            Type type = typeof(T);
            if (Entity != null)
            {
        ...
        {
            var DeletedElement = await GetByCode(code);
